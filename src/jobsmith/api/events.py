...
def _validate_slug_or_404(apps_dir: Path, slug: str) -> Path:
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail="Cannot resolve slug path")
...
# ---------------------------------------------------------------------------
# DB query helpers (operate on a borrowed connection; never close here)
# ---------------------------------------------------------------------------
...
async def stream_events(
    slug: str,
    request: Request,
    verbosity: Verbosity = Query("verbose"),
    since_run: int = Query(-1, alias="since_run"),
    since_specialist: int = Query(-1, alias="since"),
) -> EventSourceResponse:
