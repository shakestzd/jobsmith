...
def _reuse_gather_artifacts(
    *,
    matched_slug: str,
    current_slug: str,
    resolved_cwd: Path,
    apply_state_dir_fn: object,
    reuse_company_research: bool,
) -> bool:
    """Copy a prior application's gather artifacts into the current app.

    Enforces finding #2 whole-artifact reuse: copies ``jd-parsed.json`` and
    ``fit-score.json`` (and ``company-research.md`` when *reuse_company_research*)
    from *matched_slug*'s ``.apply-state/`` into *current_slug*'s.  The caller
    marks the gather phase done so the headless specialist call is skipped.

    Returns ``True`` only when BOTH required artifacts (jd-parsed + fit-score)
    were copied — correctness-first: a partial copy must not skip gather.
    Any error returns ``False`` so the pipeline regenerates normally.
    """
    import shutil

    try:
        src_dir = apply_state_dir_fn(matched_slug, resolved_cwd)  # type: ignore[operator]
        dst_dir = apply_state_dir_fn(current_slug, resolved_cwd)  # type: ignore[operator]
        if src_dir is None or dst_dir is None:
            return False

        required = ("jd-parsed.json", "fit-score.json")
        if not all((src_dir / name).exists() for name in required):
            logger.warning(
                "reuse: matched %s missing required gather artifacts — "
                "regenerating",
                matched_slug,
            )
            return False
        ...
        if reuse_company_research:
            cr = src_dir / "company-research.md"
            if cr.exists():
                shutil.copy2(cr, dst_dir / "company-research.md")

        return True
    except Exception as exc:  # noqa: BLE001 — reuse must never abort the apply
        logger.warning("reuse: failed to copy gather artifacts — regenerating: %s", exc)
        return False
...
def _run_apply_phases(
    *,
    ...
    # Finding #2 enforcement (bug-6204cdad): when the plan found a near-duplicate
    # prior application (matched_slug) and decided to REUSE the gather-phase
    # whole artifacts (jd-parse + fit-score), copy those artifacts from the prior
    # app into THIS app's .apply-state/ and mark the gather phase done so the
    # phase loop skips the headless gather specialist call entirely.
    #
    # Guardrails:
    #   - no_reuse short-circuits inside _compute_pipeline_reuse_plan
    #     (matched_slug is None), so this is a no-op under --no-reuse.
    #   - Only fires when BOTH jd_parse AND fit_score say "reuse" (correctness-
    #     first: a partial decision regenerates normally).
    #   - The unconditional render-phase backstop (below) re-gates the copied
    #     output, so reused artifacts are never trusted blindly.
    #   - bullet-decisions.json is regenerated by Step 4/5 (Step 3pre) before
    #     draft, so skipping the gather specialist does not strand bullet work.
    if (
        not no_reuse
        and not phase_done.get("gather", False)
        and reuse_plan.matched_slug is not None
        and reuse_plan.jd_parse.decision == "reuse"
        and reuse_plan.fit_score.decision == "reuse"
    ):
        copied = _reuse_gather_artifacts(
            matched_slug=reuse_plan.matched_slug,
            current_slug=slug,
            resolved_cwd=resolved_cwd,
            apply_state_dir_fn=_apply_state_dir,
            reuse_company_research=(
                reuse_plan.company_research.decision == "reuse"
            ),
        )
        if copied:
            phase_done["gather"] = True
            rdr.print_info(
                f"reuse: copied gather artifacts from {reuse_plan.matched_slug} "
                f"→ skipping gather specialists (backstop still re-gates output)"
            )
