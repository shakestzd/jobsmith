...
- `"matched_requirement_hash"`: the canonical requirement hash for the requirement this bullet covers. **Do NOT read this from `jd_parsed` — the jd-parsed JSON does not carry a `content_hash` field.** Instead, obtain it from the `jobsmith reuse lookup-bullet` response: that command returns a `matched_hash` field for EVERY requirement — on both a reuse hit (`reused: true`) AND a fresh selection (`reused: false`) — so set `matched_requirement_hash` to that `matched_hash` value in both cases (this is what seeds `requirement_evidence_map` for future runs). Only leave it `null` when `matched_hash` itself is `null` (the requirement could not be hashed). The hash is computed by the Python layer over `{canonical_tag, normalized_phrase}` and stored in `canonical_requirements.content_hash`; you never compute it yourself.
